...
def config():
    with patch('{{cookiecutter.project_slug}}.settings._conf.config', dummy_settings()):
        yield
...
@pytest.fixture(scope='session')
@patch("{{cookiecutter.project_slug}}.settings.build_config", scope="session")
def manager(loop, mock_config):
    mock_config.return_value = None
    from {{cookiecutter.project_slug}}.db import get_manager, pool
...
def cli(mock_config, loop, aiohttp_client):
    app = web.Application()
    from {{cookiecutter.project_slug}}.app import make_app
